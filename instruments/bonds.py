...
class FixedRateBond:
    def __init__(self, issue_date, maturity_date, coupon, settlement_days=2,
        ...
        face_amount = 100.0
        self.issue_date = self.date_to_quantlib(issue_date)
        self.maturity_date = self.date_to_quantlib(maturity_date)
    ...
    def set_evaluation_date(self, evaluation_date):
        d = self.date_to_quantlib(evaluation_date)
    ...
    def accrued_amount(self, eval_date=None):
        ql_eval_date = self.date_to_quantlib(eval_date)
    ...
    def years_to_maturity(self, eval_date=None):
        eval_date = self.date_to_quantlib(dt.date.today()) if eval_date is None else self.date_to_quantlib(eval_date)
...
class EuropeanBond(FixedRateBond):
    def __init__(self, issue_date, maturity_date, coupon):
        super(EuropeanBond, self).__init__(
            issue_date=issue_date,
            maturity_date=maturity_date,
            coupon=coupon,
            settlement_days=2,
